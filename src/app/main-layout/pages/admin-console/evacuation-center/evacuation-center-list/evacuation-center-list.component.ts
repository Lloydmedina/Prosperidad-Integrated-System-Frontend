...
@Component({
  selector: 'app-evacuation-center-list',
  templateUrl: './evacuation-center-list.component.html',
  styleUrls: ['./evacuation-center-list.component.scss']
})
export class EvacuationCenterListComponent implements OnInit {
  ...
  ngOnInit() {
    this.control = this.checkPriv.checkPrivilege("/main/social-welfare/evacuation-setup")
  ...
  edit(id: any) {
    this.router.navigate(["/main/social-welfare/evacuation-setup/edit-form/" + id])
  }
  ...
  delete(id: any, data: any) {
    this.modal.confirm({
      ...
      nzOnOk: () => 
        this.evacuationCenterService.deleteEvacuationCenter(id).subscribe(data => {
          this.isLoading = true;
          if(data) {
            this.evacuationCenterService.getEvacuationCenter().subscribe(async data => {
              this.listOfData = data[0];
            ...
            this.notification.create(
              ...
              'Evacuation Center Record has successfully deleted.'
            );
          } else {
            this.notification.create(
              "error",
              'Unsuccessfully Deleted',
              'Evacuation Center Record unsuccessfully deleted.'
            );
          }
        })
    });
  ...
  applyFilter(){
    ...
    if (this.localStorageDeletedStatus == 0) {
      if (this.localStorageStatus == 0) {
        this.isLoading = true
        this.evacuationCenterService.getAllEvacuationCenter().subscribe(data => {
          this.isLoading = false
          this.listOfData = data[0]
          this.fcList = this.listOfData
        })
      ...
      this.isLoading = true
      this.evacuationCenterService.getEvacuationCenterDeletedOnly().subscribe(data => {
        this.isLoading = false
        this.listOfData = data[0]
        this.fcList = this.listOfData
      })
  ...
  triggerSearch(value:any){
    ...
    if(value.tag == 'vn'){
      this.listOfData = this.fcList.filter((obj:any) => obj.venue.toLowerCase() == value.val.toLowerCase());
  ...
  clearSearch(){
    ...
    if (this.localStatusClearDeleted == 0) {
      if (this.localStatusClear == 0) {
        this.evacuationCenterService.getAllEvacuationCenter().subscribe((value:any) => {
          this.isLoading = false
        ...
        this.evacuationCenterService.getEvacuationCenter().subscribe((value:any) => {
          this.isLoading = false
          this.listOfData = value[0]
          this.fcList = this.listOfData
          this.suggestions = value[2]
  ...
  generate(value: any) {
    ...
    this.isLoading = true;
    this.evacuationCenterService.getEvacuationCenterGenerated(value.filter_type, this.localStatusGenerate, this.localStatusGenerateDeleted, value.thisMonth, value.thisYear, value.monthly, value.monthlyYear, value.yearQuarterly, value.quarter, value.yearly, value.from, value.to).subscribe(data => {
