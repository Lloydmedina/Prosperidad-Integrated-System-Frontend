...
@Component({
  selector: 'app-exhumation-permit-list',
  templateUrl: './exhumation-permit-list.component.html',
  styleUrls: ['./exhumation-permit-list.component.scss']
})
export class ExhumationPermitListComponent implements OnInit {
  ...
  txt_amount:any;
  selectedValue = null;
  basepath = "exhumation-permit-form-print";
  basepath_arr : any;
  param: any;
  drawerVisible = false;
  paymentDrawer = false;
  ...
  loadTransactionList(){
    this.exPermitServices.getList().subscribe( value =>{
 ...
 proccedToPrintCard( data_id : any) {
  ...
  delete(data : any) {
    ...
    this.modal.confirm({
      ...
      nzOnOk: () => {
        this.isLoading = true;
        this.exPermitServices.deleteTrans(data.exp_pk_id).subscribe( async x =>{
          console.log(x)
          this.loadTransactionList()
...
selectedPayment : any
  ...
  saveConfirm(value:any): void {
    ...
    this.modal.confirm({
      ...
      nzOnOk: () =>{
        this.isLoading = true;
      this.exPermitServices.payedTransactions(value.main_pk_id).subscribe(res =>{
          if (res) {
            console.log(res)
  ...
  edit(data:any){
    console.log("DATA", data.exp_pk_id)
    this.router.navigate(["/main/health/exhumation-permit/edit-form/"+data.exp_person_id+"/"+ data.exp_pk_id])
  }
