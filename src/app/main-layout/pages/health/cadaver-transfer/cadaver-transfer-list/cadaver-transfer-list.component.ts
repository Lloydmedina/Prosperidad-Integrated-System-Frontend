...
@Component({
  selector: 'app-cadaver-transfer-list',
  templateUrl: './cadaver-transfer-list.component.html',
  styleUrls: ['./cadaver-transfer-list.component.scss']
})
export class CadaverTransferListComponent implements OnInit {

  listURL = "/main/health/cadaver-transfer"
  addURL = "/main/health/cadaver-transfer/add-form"
  newURL = "/main/health/cadaver-transfer/add-form/"
  ...
  txt_amount:any;
  selectedValue = null;
  basepath = "cadaver-transfer-form-print";
  basepath_arr : any;
  param: any;
  drawerVisible = false;
  paymentDrawer = false;
 ...
 proccedToPrintCard( data_id : any) {
  ...
  delete(data : any) {
    this.modal.confirm({
      ...
      nzOnOk: () => {
        this.isLoading = true;
        this.ctpServices.deleteTrans(data.ctp_pk_id).subscribe( async x =>{
          console.log(x)
          this.getDataList()
...
selectedPayment : any
  ...
  saveConfirm(value:any): void {
    ...
    this.modal.confirm({
      ...
      nzOnOk: () =>{
        this.isLoading = true;
      this.ctpServices.payedTransactions(value.main_pk_id).subscribe(res =>{
          if (res) {
            console.log(res)
  ...
  edit(data:any){
    if(data.ctp_exhumation_id == ''|| data.ctp_exhumation_id == null){
      this.editRoutes('non Exhumated',data.ctp_pk_id);
    }else{
      data.ctp_exhumation_id
      this.editRoutes(data.ctp_exhumation_id,data.ctp_pk_id);
    }
  }
  editRoutes(ctp_id : any, ctp_pk : any){
    this.router.navigate(["/main/health/cadaver-transfer/edit-form/" +ctp_id+"/"+ctp_pk])
  }
