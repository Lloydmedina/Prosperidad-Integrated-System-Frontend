...
@Component({
  selector: 'app-dispatching-form',
  templateUrl: './dispatching-form.component.html',
  styleUrls: ['./dispatching-form.component.scss']
})
export class DispatchingFormComponent implements OnInit {
  ...
  ngOnInit() { 
    ...
    if(!this.param){
      ...
      this.dispatchService.getById(this.param).subscribe(data => {
        console.log(data)
    ...
    this.dispatchService.getDropdown().subscribe(data => {
    ...
    if(!this.param)
    {
      this.modal.confirm({
        ...
        nzOnOk: () => {
          ...
          this.dispatchService.insert(value).subscribe(async (data: any) => {
            ...
            await this.notification.create(
              "success",
              'Successfully Saved',
              'Dispatch Record has successfully saved.'
            );
          },
          async error => {
           await this.notification.create(
              ...
              'Dispatch Record has not been saved.'
            );
          });
        }
      })
      ...
      this.modal.confirm({
        ...
        nzOnOk: () => {
          this.saveLoading = true;
          this.dispatchService.update(this.param, value).subscribe(async (data: any) => {
            this.saveLoading = false;
          localStorage.removeItem('receiving-data')
          this.router.navigate(["/"+this.path[0]+"/"+this.path[1]+"/"+this.path[2] + "/" + this.path[3]]);
            await this.notification.create(
              ...
              'Dispatch Record has successfully updated.'
            );
          },
          async error => {
           await this.notification.create(
              "error",
              'Unsuccessfully Saved',
              'Dispatch Record has not been updated.'
            );
          });
        }
      });
  ...
  handleOk(){
    ...
    if(this.receivingId != ''){
      this.dispatchService.inspect(this.arrayList().at(this.recordIndex).value.animal_inspection).subscribe(data => {
        console.log(data)
      })
    }
    this.clearInspect()
