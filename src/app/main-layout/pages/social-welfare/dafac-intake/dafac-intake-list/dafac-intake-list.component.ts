...
@Component({
  selector: 'app-dafac-intake-list',
  templateUrl: './dafac-intake-list.component.html',
  styleUrls: ['./dafac-intake-list.component.scss']
})
export class DafacIntakeListComponent implements OnInit {
  ...
  ngOnInit() {
    this.control = this.checkPriv.checkPrivilege("/main/social-welfare/dafac-intake")
  ...
  delete(id: any, data: any) {
    this.modal.confirm({
      ...
      nzOnOk: () => 
        this.dafacIntakeService.deleteDafacIntake(id).subscribe(data => {
          this.isLoading = true;
          if(data) {
            this.dafacIntakeService.getDafacIntake().subscribe(async data => {
              this.listOfData = data[0];
            ...
            this.notification.create(
              ...
              'DAFAC Intake Record has successfully deleted.'
            );
          } else {
            this.notification.create(
              "error",
              'Unsuccessfully Deleted',
              'DAFAC Intake Record unsuccessfully deleted.'
            );
          }
        })
    });
  ...
  onExpandChange(id: number, checked: boolean, index: any): void {
    if (checked) {
      ...
      this.dafacIntakeService.getDafacIntakeDetails(id).subscribe(async data => {
  ...
  applyFilter(){
    ...
    if (this.localStorageDeletedStatus == 0) {
      if (this.localStorageStatus == 0) {
        this.isLoading = true
        this.dafacIntakeService.getAllDafacIntake().subscribe(data => {
          this.isLoading = false
          this.listOfData = data[0]
          this.fcList = this.listOfData
        })
      ...
      this.isLoading = true
      this.dafacIntakeService.getDafacIntakeDeletedOnly().subscribe(data => {
        this.isLoading = false
        this.listOfData = data[0]
        this.fcList = this.listOfData
      })
  ...
  clearSearch(){
    ...
    if (this.localStatusClearDeleted == 0) {
      if (this.localStatusClear == 0) {
        this.dafacIntakeService.getAllDafacIntake().subscribe((value:any) => {
          this.isLoading = false
        ...
        this.dafacIntakeService.getDafacIntake().subscribe((value:any) => {
          this.isLoading = false
          this.listOfData = value[0]
          this.fcList = this.listOfData
          this.suggestions = value[2]
  ...
  generate(value: any) {
    ...
    this.isLoading = true;
    this.dafacIntakeService.getDafacIntakeGenerated(value.filter_type, this.localStatusGenerate, this.localStatusGenerateDeleted, value.thisMonth, value.thisYear, value.monthly, value.monthlyYear, value.yearQuarterly, value.quarter, value.yearly, value.from, value.to).subscribe(data => {
