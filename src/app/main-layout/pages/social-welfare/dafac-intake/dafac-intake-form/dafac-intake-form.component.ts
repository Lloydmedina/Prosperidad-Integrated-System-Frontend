...
@Component({
  selector: 'app-dafac-intake-form',
  templateUrl: './dafac-intake-form.component.html',
  styleUrls: ['./dafac-intake-form.component.scss']
})
export class DafacIntakeFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dafacIntakeService.getEducationalType().subscribe(data => {
    ...
    if (this.param) {
      ...
      this.isLoading = true;
      this.dafacIntakeService.getDafacIntakeEdit(this.param).subscribe((main_data: any) => {
        console.log("MAIN", main_data)
        this.isLoading = false;
        this.mainPersonGuid = main_data.person_guid;
        this.validateForm.patchValue({
          monthly_income: main_data.monthly_family_income,
          fourps_member: main_data.fourps_beneficiary,
          ips: main_data.ips,
      ...
      if (this.addParam) {
        this.isLoading = true;
        this.dafacIntakeService.getAddPerson(this.addParam).subscribe((data: any) => {
          this.isLoading = false;
          this.validateForm.patchValue({
            prefix: data.prefix,
  ...
  cancel(): void {
    this.router.navigate(['/main/social-welfare/dafac-intake'])
  }
  ...
  submitForm(): void {
    ...
    if(!this.param) {
      ...
      this.dafacIntakeService.saveDafacIntake(this.validateForm.value).subscribe(data => {
        if (data) {
          this.saveLoading = false;
          this.router.navigate(['/' + this.path[0] + '/' + this.path[1] + '/' + this.path[2]])
            this.notification.create(
              "success",
              'Successfully Saved',
              'DAFAC Intake Setup has successfully saved.'
            );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'DAFAC Intake Setup unsuccessfully saved.'
          );
        }
      }, error => {
        this.saveLoading = false;
      ...
      this.dafacIntakeService.updateDafacIntake(this.param, this.validateForm.value).subscribe(data => {
        this.isLoading = false;
        if (data) {
          this.saveLoading = false;
          this.router.navigate(['/' + this.path[0] + '/' + this.path[1] + '/' + this.path[2]])
          this.notification.create(
            ...
            'DAFAC Intake Setup has successfully updated.'
          );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'DAFAC Intake Setup unsuccessfully updated.'
          );
        }
      }, error => {
        this.saveLoading = false;
