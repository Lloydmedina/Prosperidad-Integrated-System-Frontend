...
@Component({
  selector: 'app-wcc-ct-form-print',
  templateUrl: './wcc-ct-form-print.component.html',
  styleUrls: ['./wcc-ct-form-print.component.scss']
})
export class WccCtFormPrintComponent implements OnInit {
  ...
  loadDatas(id : any){
    ...
    this.wccTOCServices.getData(id).subscribe(data => {
      this.tocData = data[0][0];

     this.wtnsData1 = this.tocData.witness_data[0];
      this.wtnsData2 = this.tocData.witness_data[1];
      this.isLoading = false;
      this.transDate = moment(this.tocData.wcc_toc_transdate).format();
      console.log("data",this.wtnsData2.wcc_toc_witness_pname);
      this.getFamilyId(this.tocData.client_pid);
  ...
  convertToPDF() {
    ...
    html2canvas(this.dataElement).then(canvas => {
      ...
      pdf.save("WCC_CUSTODY_TURNOVER.pdf"); // Generated PDF
    });
  }
