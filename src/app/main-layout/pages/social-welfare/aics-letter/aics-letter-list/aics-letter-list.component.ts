...
@Component({
  selector: 'app-aics-letter-list',
  templateUrl: './aics-letter-list.component.html',
  styleUrls: ['./aics-letter-list.component.scss']
})
export class AicsLetterListComponent implements OnInit {
  ...
  ngOnInit() {
    this.control = this.checkPriv.checkPrivilege("/main/social-welfare/aics-letter")
  ...
  edit(id: any) {
    this.router.navigate(["/main/social-welfare/aics-letter/edit-form/" + id])
  }
  ...
  delete(id: any, data: any) {
    this.modal.confirm({
      ...
      nzOnOk: () => 
        this.aicsLetterService.deleteAicsLetter(id).subscribe(data => {
          this.isLoading = true;
          if(data) {
            this.aicsLetterService.getAicsLetter().subscribe(async data => {
              this.listOfData = data[0];
            ...
            this.notification.create(
              ...
              'AICS Letter has successfully deleted.'
            );
          } else {
            this.notification.create(
              "error",
              'Unsuccessfully Deleted',
              'AICS Letter unsuccessfully deleted.'
            );
          }
        })
    });
  ...
  print(id: any) {
    this.router.navigate(["/main/social-welfare/aics-letter/print-form/" + id])
  }
  ...
  applyFilter(){
    ...
    if (this.localStorageDeletedStatus == 0) {
      if (this.localStorageStatus == 0) {
        this.isLoading = true
        this.aicsLetterService.getAllAicsLetter().subscribe(data => {
          this.isLoading = false
          this.listOfData = data[0]
          this.fcList = this.listOfData
        })
      ...
      this.isLoading = true
      this.aicsLetterService.getAicsLetterDeletedOnly().subscribe(data => {
        this.isLoading = false
        this.listOfData = data[0]
        this.fcList = this.listOfData
      })
  ...
  clearSearch(){
    ...
    if (this.localStatusClearDeleted == 0) {
      if (this.localStatusClear == 0) {
        this.aicsLetterService.getAllAicsLetter().subscribe((value:any) => {
          this.isLoading = false
        ...
        this.aicsLetterService.getAicsLetter().subscribe((value:any) => {
          this.isLoading = false
          this.listOfData = value[0]
          this.fcList = this.listOfData
          this.suggestions = value[2]
  ...
  generate(value: any) {
    ...
    this.isLoading = true;
    this.aicsLetterService.getAicsLetterGenerated(value.filter_type, this.localStatusGenerate, this.localStatusGenerateDeleted, value.thisMonth, value.thisYear, value.monthly, value.monthlyYear, value.yearQuarterly, value.quarter, value.yearly, value.from, value.to).subscribe(data => {
