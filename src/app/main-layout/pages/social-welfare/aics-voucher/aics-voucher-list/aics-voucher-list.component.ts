...
@Component({
  selector: 'app-aics-voucher-list',
  templateUrl: './aics-voucher-list.component.html',
  styleUrls: ['./aics-voucher-list.component.scss']
})
export class AicsVoucherListComponent implements OnInit {
  ...
  edit(id: any) {
    this.router.navigate(["/main/social-welfare/aics-voucher/edit-form/" + id])
  }
  ...
  delete(id: any, data: any) {
    this.modal.confirm({
      ...
      nzOnOk: () => 
        this.aicsVoucherService.deleteAicsVoucher(id).subscribe(data => {
          this.isLoading = true;
          if(data) {
            this.aicsVoucherService.getAicsVoucher().subscribe(async data => {
              this.listOfData = data[0];
            ...
            this.notification.create(
              ...
              'AICS Voucher has successfully deleted.'
            );
          } else {
            this.notification.create(
              "error",
              'Unsuccessfully Deleted',
              'AICS Voucher unsuccessfully deleted.'
            );
          }
        })
    });
  ...
  print(id: any) {
    this.router.navigate(["/main/social-welfare/aics-voucher/print-form/" + id])
  }
  ...
  applyFilter(){
    ...
    if (this.localStorageDeletedStatus == 0) {
      if (this.localStorageStatus == 0) {
        this.isLoading = true
        this.aicsVoucherService.getAllAicsVoucher().subscribe(data => {
          this.isLoading = false
          this.listOfData = data[0]
          this.fcList = this.listOfData
        })
      ...
      this.isLoading = true
      this.aicsVoucherService.getAicsVoucherDeletedOnly().subscribe(data => {
        this.isLoading = false
        this.listOfData = data[0]
        this.fcList = this.listOfData
      })
  ...
  clearSearch(){
    ...
    if (this.localStatusClearDeleted == 0) {
      if (this.localStatusClear == 0) {
        this.aicsVoucherService.getAllAicsVoucher().subscribe((value:any) => {
          this.isLoading = false
        ...
        this.aicsVoucherService.getAicsVoucher().subscribe((value:any) => {
          this.isLoading = false
          this.listOfData = value[0]
          this.fcList = this.listOfData
          this.suggestions = value[2]
  ...
  generate(value: any) {
    ...
    this.isLoading = true;
    this.aicsVoucherService.getAicsVoucherGenerated(value.filter_type, this.localStatusGenerate, this.localStatusGenerateDeleted, value.thisMonth, value.thisYear, value.monthly, value.monthlyYear, value.yearQuarterly, value.quarter, value.yearly, value.from, value.to).subscribe(data => {
