...
@Component({
  selector: 'app-pwd-list',
  templateUrl: './pwd-list.component.html',
  styleUrls: ['./pwd-list.component.scss']
})
export class PwdListComponent implements OnInit {
  ...
  ngOnInit() {
    this.control = this.checkPriv.checkPrivilege("/main/social-welfare/pwd-registration")
  ...
  delete(id: any, data: any) {
    this.modal.confirm({
      ...
      nzOnOk: () => 
        this.pwdService.deletePwd(id).subscribe(data => {
          this.isLoading = true;
          if(data) {
            this.pwdService.getPwd().subscribe(async data => {
              this.listOfData = data[0];
            ...
            this.notification.create(
              ...
              'PWD Record has successfully deleted.'
            );
          } else {
            this.notification.create(
              "error",
              'Unsuccessfully Deleted',
              'PWD Record unsuccessfully deleted.'
            );
          }
        })
    });
  ...
  onExpandChange(id: number, checked: boolean, index: any): void {
    if (checked) {
      ...
      this.pwdService.getPwdDetails(id).subscribe(async data => {
  ...
  applyFilter(){
    ...
    if (this.localStorageDeletedStatus == 0) {
      ...
      this.isLoading = true
      this.pwdService.getPwdDeletedOnly().subscribe(data => {
        this.isLoading = false
        this.listOfData = data[0]
        this.fcList = this.listOfData
      })
  ...
  clearSearch(){
    ...
    if (this.localStatusClearDeleted == 0) {
      if (this.localStatusClear == 0) {
        this.pwdService.getAllPwd().subscribe((value:any) => {
          this.isLoading = false
        ...
        this.pwdService.getPwd().subscribe((value:any) => {
          this.isLoading = false
          this.listOfData = value[0]
          this.fcList = this.listOfData
          this.suggestions = value[2]
  ...
  generate(value: any) {
    ...
    this.isLoading = true;
    this.pwdService.getPwdGenerated(value.filter_type, this.localStatusGenerate, this.localStatusGenerateDeleted, value.thisMonth, value.thisYear, value.monthly, value.monthlyYear, value.yearQuarterly, value.quarter, value.yearly, value.from, value.to).subscribe(data => {
