...
@Component({
  selector: 'app-child-intake-form',
  templateUrl: './child-intake-form.component.html',
  styleUrls: ['./child-intake-form.component.scss']
})
export class ChildIntakeFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.validateForm = this.fb.group({
      ...
      secondary_contact_no: [null, [Validators.required]],
      ...
      total_family_income: [null, [Validators.required]],
      owner: [null, [Validators.required]],
      renter: [null, [Validators.required]],
      estimated_damaged: [null, [Validators.required]],
      if_distressed: [null, [Validators.required]],
      physical_disability: [null, [Validators.required]],
      type_of_disability_id: [null, [Validators.required]],
      sources_of_income: [null, [Validators.required]],
      no_of_hectares: [null, [Validators.required]],
      crops_planted:  [null, [Validators.required]],
      area_of_location:  [null, [Validators.required]],
      other_sources_of_income:  [null, [Validators.required]],
      member_count: [null],
      form_trans_no: [null],
    ...
    this.childIntakeService.getEducationalType().subscribe(data => {
    ...
    if (this.param) {
      ...
      this.isLoading = true;
      this.childIntakeService.getChildIntakeEdit(this.param).subscribe((main_data: any) => {
        this.isLoading = false;
      ...
      if (this.addParam) {
        this.isLoading = true;
        this.childIntakeService.getAddPerson(this.addParam).subscribe((data: any) => {
          this.isLoading = false;
          console.log("BUHASY", data)
          this.validateForm.patchValue({
            prefix: data.prefix,
  ...
  submitForm(): void {
    ...
    if(!this.param) {
      ...
      this.childIntakeService.saveChildIntake(this.validateForm.value).subscribe(data => {
        if (data) {
          ...
          this.router.navigate(['/' + this.path[0] + '/' + this.path[1] + '/' + this.path[2]])
            this.notification.create(
              "success",
              'Successfully Saved',
              'Child Intake Setup has successfully saved.'
            );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'Child Intake Setup unsuccessfully saved.'
          );
        }
      }, error => {
        this.saveLoading = false;
      ...
      this.childIntakeService.updateChildIntake(this.param, this.validateForm.value).subscribe(data => {
        this.isLoading = false;
        if (data) {
          this.saveLoading = false;
          this.router.navigate(['/' + this.path[0] + '/' + this.path[1] + '/' + this.path[2]])
          this.notification.create(
            ...
            'Child Intake Setup has successfully updated.'
          );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'Child Intake Setup unsuccessfully updated.'
          );
        }
      }, error => {
        this.saveLoading = false;
