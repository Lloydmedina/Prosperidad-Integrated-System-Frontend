...
@Component({
  selector: 'app-osca-registration-list',
  templateUrl: './osca-registration-list.component.html',
  styleUrls: ['./osca-registration-list.component.scss']
})
export class OscaRegistrationListComponent implements OnInit {
  ...
  ngOnInit() {
    this.control = this.checkPriv.checkPrivilege("/main/social-welfare/osca-registration")
  ...
  delete(id: any, data: any) {
    this.modal.confirm({
      ...
      nzOnOk: () => 
        this.oscaRegistrationService.deleteOscaRegistration(id).subscribe(data => {
          this.isLoading = true;
          if(data) {
            this.oscaRegistrationService.getOscaRegistration().subscribe(async data => {
              this.listOfData = data[0];
            ...
            this.notification.create(
              ...
              'OSCA Registered Record has successfully deleted.'
            );
          } else {
            this.notification.create(
              "error",
              'Unsuccessfully Deleted',
              'OSCA Registered Record unsuccessfully deleted.'
            );
          }
        })
    });
  ...
  applyFilter(){
    ...
    if (this.localStorageDeletedStatus == 0) {
      ...
      this.isLoading = true
      this.oscaRegistrationService.getOscaRegistrationDeletedOnly().subscribe(data => {
        this.isLoading = false
        this.listOfData = data[0]
        this.fcList = this.listOfData
      })
  ...
  clearSearch(){
    ...
    if (this.localStatusClearDeleted == 0) {
      if (this.localStatusClear == 0) {
        this.oscaRegistrationService.getAllOscaRegistration().subscribe((value:any) => {
          this.isLoading = false
  ...
  generate(value: any) {
    ...
    this.isLoading = true;
    this.oscaRegistrationService.getOscaRegistrationGenerated(value.filter_type, this.localStatusGenerate, this.localStatusGenerateDeleted, value.thisMonth, value.thisYear, value.monthly, value.monthlyYear, value.yearQuarterly, value.quarter, value.yearly, value.from, value.to).subscribe(data => {
