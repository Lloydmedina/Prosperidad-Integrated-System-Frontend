...
@Component({
  selector: 'app-general-intake-form',
  templateUrl: './general-intake-form.component.html',
  styleUrls: ['./general-intake-form.component.scss']
})
export class GeneralIntakeFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.generalIntakeService.getEducationalType().subscribe(data => {
    ...
    if (this.param) {
      ...
      this.isLoading = true;
      this.generalIntakeService.getGeneralIntakeEdit(this.param).subscribe((main_data: any) => {
        this.isLoading = false;
        this.mainPersonGuid = main_data.person_guid;
        console.log("AHAK", main_data)
        this.validateForm.patchValue({
      ...
      if (this.addParam) {
        this.generalIntakeService.getAddPerson(this.addParam).subscribe((data: any) => {
          this.validateForm.patchValue({
            prefix: data.prefix,
  ...
  cancel(): void {
    this.router.navigate(['/main/social-welfare/fc-intake'])
  }
  ...
  submitForm(): void {
    ...
    if(!this.param) {
      ...
      this.generalIntakeService.saveGeneralIntake(this.validateForm.value).subscribe(data => {
        if (data) {
          this.saveLoading = false;
          this.router.navigate(['/' + this.path[0] + '/' + this.path[1] + '/' + this.path[2]])
            this.notification.create(
              "success",
              'Successfully Saved',
              'FC Intake Setup has successfully saved.'
            );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'FC Intake Setup unsuccessfully saved.'
          );
        }
      }, error => {
        this.saveLoading = false;
      ...
      this.generalIntakeService.updateGeneralIntake(this.param, this.validateForm.value).subscribe(data => {
        this.isLoading = false;
        if (data) {
          this.saveLoading = false;
          this.router.navigate(['/' + this.path[0] + '/' + this.path[1] + '/' + this.path[2]])
          this.notification.create(
            ...
            'FC Intake Setup has successfully updated.'
          );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'FC Intake Setup unsuccessfully updated.'
          );
        }
      }, error => {
        this.saveLoading = false;
