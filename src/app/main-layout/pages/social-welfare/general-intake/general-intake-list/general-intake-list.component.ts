...
@Component({
  selector: 'app-general-intake-list',
  templateUrl: './general-intake-list.component.html',
  styleUrls: ['./general-intake-list.component.scss']
})
export class GeneralIntakeListComponent implements OnInit {
  ...
  ngOnInit() {
    this.control = this.checkPriv.checkPrivilege("/main/social-welfare/fc-intake")
  ...
  edit(id: any) {
    this.router.navigate(["/main/social-welfare/fc-intake/edit-form/" + id])
  }
  ...
  delete(id: any, data: any) {
    this.modal.confirm({
      ...
      nzOnOk: () => 
        this.generalIntakeService.deleteGeneralIntake(id).subscribe(data => {
          this.isLoading = true;
          if(data) {
            this.generalIntakeService.getGeneralIntake().subscribe(async data => {
              this.listOfData = data[0];
            ...
            this.notification.create(
              ...
              'General Intake Record has successfully deleted.'
            );
          } else {
            this.notification.create(
              "error",
              'Unsuccessfully Deleted',
              'General Intake Record unsuccessfully deleted.'
            );
          }
        })
    });
  ...
  print(id: any) {
    this.router.navigate(["/main/social-welfare/fc-intake/print-form/" + id])
  }

  onExpandChange(id: number, checked: boolean, index: any): void {
    if (checked) {
      this.expandSet.add(id);
      this.generalIntakeService.getGeneralIntakeDetails(id).subscribe(async data => {
  ...
  applyFilter(){
    ...
    if (this.localStorageDeletedStatus == 0) {
      if (this.localStorageStatus == 0) {
        this.isLoading = true
        this.generalIntakeService.getAllGeneralIntake().subscribe(data => {
          this.isLoading = false
          this.listOfData = data[0]
          this.fcList = this.listOfData
        })
      ...
      this.isLoading = true
      this.generalIntakeService.getGeneralIntakeDeletedOnly().subscribe(data => {
        this.isLoading = false
        this.listOfData = data[0]
        this.fcList = this.listOfData
      })
  ...
  clearSearch(){
    ...
    if (this.localStatusClearDeleted == 0) {
      if (this.localStatusClear == 0) {
        this.generalIntakeService.getAllGeneralIntake().subscribe((value:any) => {
          this.isLoading = false
        ...
        this.generalIntakeService.getGeneralIntake().subscribe((value:any) => {
          this.isLoading = false
          this.listOfData = value[0]
          this.fcList = this.listOfData
          this.suggestions = value[2]
  ...
  generate(value: any) {
    ...
    this.isLoading = true;
    this.generalIntakeService.getGeneralIntakeGenerated(value.filter_type, this.localStatusGenerate, this.localStatusGenerateDeleted, value.thisMonth, value.thisYear, value.monthly, value.monthlyYear, value.yearQuarterly, value.quarter, value.yearly, value.from, value.to).subscribe(data => {
