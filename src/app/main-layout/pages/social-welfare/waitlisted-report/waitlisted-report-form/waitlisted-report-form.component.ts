...
@Component({
  selector: 'app-waitlisted-report-form',
  templateUrl: './waitlisted-report-form.component.html',
  styleUrls: ['./waitlisted-report-form.component.scss']
})
export class WaitlistedReportFormComponent implements OnInit {
   ...
   ngOnInit() {
    ...
    if (this.param) {
      ...
      if (this.addParam) {
        ...
        this.oscaIntakeService.getOscaIntakeEdit(this.addParam).subscribe((data: any) => {
          this.isLoading = false;
  ...
  cancel(): void {
    this.router.navigate(['/main/social-welfare/waitlisted-report'])
  }
  ...
  submitForm(): void {
    ...
    this.validateForm.value.osca_intake_guid = this.intakeData.osca_intake_guid
    this.validateForm.value.person_guid = this.intakeData.person_guid

    if(!this.param) {
      this.saveLoading = true;
      // console.log("YAKS", this.validateForm.value)
      // console.log("YAKS 2", this.intakeData)
      this.waitlistedReportService.saveWaitlistedReport(this.validateForm.value).subscribe(data => {
        if (data) {
          ...
          this.router.navigate(['/main/social-welfare/waitlisted-report'])
            this.notification.create(
              "success",
              'Successfully Saved',
              'Waitlisted Report has successfully saved.'
            );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'Waitlisted Report unsuccessfully saved.'
          );
        }
      }, error => {
        this.saveLoading = false;
      ...
      this.waitlistedReportService.updateWaitlistedReport(this.param, this.validateForm.value).subscribe(data => {
        this.isLoading = false;
        if (data) {
          this.saveLoading = false;
          this.router.navigate(['/main/social-welfare/waitlisted-report'])
          this.notification.create(
            ...
            'Waitlisted Report has successfully updated.'
          );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'Waitlisted Report unsuccessfully updated.'
          );
        }
      }, error => {
        this.saveLoading = false;
