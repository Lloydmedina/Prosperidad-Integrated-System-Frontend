...
export class SpListComponent implements OnInit {
  ...
  ngOnInit() {
    this.control = this.checkPriv.checkPrivilege("/main/social-welfare/solo-parent-registration")
  ...
  delete(id: any, data: any) {
    this.modal.confirm({
      ...
      nzOnOk: () => 
        this.soloParentService.deleteSoloParent(id).subscribe(data => {
          this.isLoading = true;
          if(data) {
            this.soloParentService.getSoloParent().subscribe(async data => {
              this.listOfData = data[0];
            ...
            this.notification.create(
              ...
              'Solo Parent Record has successfully deleted.'
            );
          } else {
            this.notification.create(
              "error",
              'Unsuccessfully Deleted',
              'Solo Parent Record unsuccessfully deleted.'
            );
          }
        })
    });
  ...
  onExpandChange(id: number, checked: boolean, index: any): void {
    if (checked) {
      ...
      this.soloParentService.getSoloParentDetails(id).subscribe(async data => {
  ...
  applyFilter(){
    ...
    if (this.localStorageDeletedStatus == 0) {
      if (this.localStorageStatus == 0) {
        this.isLoading = true
        this.soloParentService.getAllSoloParent().subscribe(data => {
          this.isLoading = false
          this.listOfData = data[0]
          this.fcList = this.listOfData
        })
      ...
      this.isLoading = true
      this.soloParentService.getSoloParentDeletedOnly().subscribe(data => {
        this.isLoading = false
        this.listOfData = data[0]
        this.fcList = this.listOfData
      })
  ...
  clearSearch(){
    ...
    if (this.localStatusClearDeleted == 0) {
      if (this.localStatusClear == 0) {
        this.soloParentService.getAllSoloParent().subscribe((value:any) => {
          this.isLoading = false
        ...
        this.soloParentService.getSoloParent().subscribe((value:any) => {
          this.isLoading = false
          this.listOfData = value[0]
          this.fcList = this.listOfData
          this.suggestions = value[2]
  ...
  generate(value: any) {
    ...
    this.isLoading = true;
    this.soloParentService.getSoloParentGenerated(value.filter_type, this.localStatusGenerate, this.localStatusGenerateDeleted, value.thisMonth, value.thisYear, value.monthly, value.monthlyYear, value.yearQuarterly, value.quarter, value.yearly, value.from, value.to).subscribe(data => {
