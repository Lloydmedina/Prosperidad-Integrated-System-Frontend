...
@Component({
  selector: 'app-sp-intake-form',
  templateUrl: './sp-intake-form.component.html',
  styleUrls: ['./sp-intake-form.component.scss']
})
export class SpIntakeFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.spIntakeService.getEducationalType().subscribe(data => {
    ...
    if (this.param) {
      ...
      this.isLoading = true;
      this.spIntakeService.getSoloParentIntakeEdit(this.param).subscribe((main_data: any) => {
        this.isLoading = false;
      ...
      if (this.addParam) {
        this.isLoading = true;
        this.spIntakeService.getAddPerson(this.addParam).subscribe((data: any) => {
          this.isLoading = false;
  ...
  submitForm(): void {
    ...
    if(!this.param) {
      ...
      this.spIntakeService.saveSoloParentIntake(this.validateForm.value).subscribe(data => {
        if (data) {
          ...
          this.router.navigate(['/' + this.path[0] + '/' + this.path[1] + '/' + this.path[2]])
            this.notification.create(
              "success",
              'Successfully Saved',
              'Solo Parent Intake Setup has successfully saved.'
            );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'Solo Parent Intake Setup unsuccessfully saved.'
          );
        }
      }, error => {
        this.saveLoading = false;
      ...
      this.spIntakeService.updateSoloParentIntake(this.param, this.validateForm.value).subscribe(data => {
        this.isLoading = false;
        if (data) {
          this.saveLoading = false;
          this.router.navigate(['/' + this.path[0] + '/' + this.path[1] + '/' + this.path[2]])
          this.notification.create(
            ...
            'Solo Parent Intake Setup has successfully updated.'
          );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'Solo Parent Intake Setup unsuccessfully updated.'
          );
        }
      }, error => {
        this.saveLoading = false;
