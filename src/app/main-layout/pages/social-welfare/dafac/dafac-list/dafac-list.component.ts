...
@Component({
  selector: 'app-dafac-list',
  templateUrl: './dafac-list.component.html',
  styleUrls: ['./dafac-list.component.scss']
})
export class DafacListComponent implements OnInit {
  ...
  ngOnInit() {
    this.control = this.checkPriv.checkPrivilege("/main/social-welfare/dafac-registration")
  ...
  delete(id: any, data: any) {
    this.modal.confirm({
      ...
      nzOnOk: () => 
        this.dafacService.deleteDafac(id).subscribe(data => {
          this.isLoading = true;
          if(data) {
            this.dafacService.getDafac().subscribe(async data => {
              this.listOfData = data[0];
            ...
            this.notification.create(
              ...
              'Dafac Record has successfully deleted.'
            );
          } else {
            this.notification.create(
              "error",
              'Unsuccessfully Deleted',
              'Dafac Record unsuccessfully deleted.'
            );
          }
        })
    });
  ...
  onExpandChange(id: number, checked: boolean, index: any): void {
    if (checked) {
      ...
      this.dafacService.getGeneralDafacDetails(id).subscribe(async data => {
  ...
  applyFilter(){
    ...
    if (this.localStorageDeletedStatus == 0) {
      ...
      this.isLoading = true
      this.dafacService.getDafacDeletedOnly().subscribe(data => {
        this.isLoading = false
        this.listOfData = data[0]
        this.fcList = this.listOfData
      })
  ...
  clearSearch(){
    ...
    if (this.localStatusClearDeleted == 0) {
      if (this.localStatusClear == 0) {
        this.dafacService.getAllDafac().subscribe((value:any) => {
          this.isLoading = false
        ...
        this.dafacService.getDafac().subscribe((value:any) => {
          this.isLoading = false
          this.listOfData = value[0]
          this.fcList = this.listOfData
          this.suggestions = value[2]
  ...
  generate(value: any) {
    ...
    this.isLoading = true;
    this.dafacService.getDafacGenerated(value.filter_type, this.localStatusGenerate, this.localStatusGenerateDeleted, value.thisMonth, value.thisYear, value.monthly, value.monthlyYear, value.yearQuarterly, value.quarter, value.yearly, value.from, value.to).subscribe(data => {
