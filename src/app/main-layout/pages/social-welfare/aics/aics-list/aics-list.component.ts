...
@Component({
  selector: 'app-aics-list',
  templateUrl: './aics-list.component.html',
  styleUrls: ['./aics-list.component.scss']
})
export class AicsListComponent implements OnInit {
  ...
  ngOnInit() {
    this.control = this.checkPriv.checkPrivilege("/main/social-welfare/aics-registration")
  ...
  edit(id: any) {
    this.router.navigate(["/main/social-welfare/aics-registration/edit-form/" + id])
  }
  ...
  print(id: any) {
    this.router.navigate(["/main/social-welfare/aics-registration/print-form/" + id])
  }

  onExpandChange(id: number, checked: boolean, index: any): void {
    if (checked) {
      this.expandSet.add(id);
      this.aicsService.getAicsDetails(id).subscribe(async data => {
  ...
  applyFilter(){
    ...
    if (this.localStorageDeletedStatus == 0) {
      ...
      this.isLoading = true
      this.aicsService.getAicsDeletedOnly().subscribe(data => {
        this.isLoading = false
        this.listOfData = data[0]
        this.fcList = this.listOfData
      })
  ...
  clearSearch(){
    ...
    if (this.localStatusClearDeleted == 0) {
      if (this.localStatusClear == 0) {
        this.aicsService.getAllAics().subscribe((value:any) => {
          this.isLoading = false
        ...
        this.aicsService.getAics().subscribe((value:any) => {
          this.isLoading = false
          this.listOfData = value[0]
          this.fcList = this.listOfData
          this.suggestions = value[2]
  ...
  generate(value: any) {
    ...
    this.isLoading = true;
    this.aicsService.getAicsGenerated(value.filter_type, this.localStatusGenerate, this.localStatusGenerateDeleted, value.thisMonth, value.thisYear, value.monthly, value.monthlyYear, value.yearQuarterly, value.quarter, value.yearly, value.from, value.to).subscribe(data => {
