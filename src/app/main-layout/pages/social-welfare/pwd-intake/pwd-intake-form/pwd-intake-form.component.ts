...
@Component({
  selector: 'app-pwd-intake-form',
  templateUrl: './pwd-intake-form.component.html',
  styleUrls: ['./pwd-intake-form.component.scss']
})
export class PwdIntakeFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pwdIntakeService.getEducationalType().subscribe(data => {
    ...
    if (this.param) {
      ...
      this.isLoading = true;
      this.pwdIntakeService.getPwdIntakeEdit(this.param).subscribe((main_data: any) => {
        this.isLoading = false;
      ...
      if (this.addParam) {
        this.isLoading = true;
        this.pwdIntakeService.getAddPerson(this.addParam).subscribe((data: any) => {
          this.isLoading = false;
  ...
  submitForm(): void {
    ...
    if(!this.param) {
      ...
      this.pwdIntakeService.savePwdIntake(this.validateForm.value).subscribe(data => {
        if (data) {
          ...
          this.router.navigate(['/' + this.path[0] + '/' + this.path[1] + '/' + this.path[2]])
            this.notification.create(
              "success",
              'Successfully Saved',
              'PWD Intake Setup has successfully saved.'
            );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'PWD Intake Setup unsuccessfully saved.'
          );
        }
      }, error => {
        this.saveLoading = false;
      ...
      this.pwdIntakeService.updatePwdIntake(this.param, this.validateForm.value).subscribe(data => {
        this.isLoading = false;
        if (data) {
          this.saveLoading = false;
          this.router.navigate(['/' + this.path[0] + '/' + this.path[1] + '/' + this.path[2]])
          this.notification.create(
            ...
            'PWD Intake Setup has successfully updated.'
          );
        } else {
          this.saveLoading = false;
          this.notification.create(
            ...
            'PWD Intake Setup unsuccessfully updated.'
          );
        }
      }, error => {
        this.saveLoading = false;
