...
@Component({
  selector: 'app-pwd-intake-list',
  templateUrl: './pwd-intake-list.component.html',
  styleUrls: ['./pwd-intake-list.component.scss']
})
export class PwdIntakeListComponent implements OnInit {
  ...
  ngOnInit() {
    this.control = this.checkPriv.checkPrivilege("/main/social-welfare/pwd-intake")
  ...
  delete(id: any, data: any) {
    this.modal.confirm({
      ...
      nzOnOk: () => 
        this.pwdIntakeService.deletePwdIntake(id).subscribe(data => {
          this.isLoading = true;
          if(data) {
            this.pwdIntakeService.getPwdIntake().subscribe(async data => {
              this.listOfData = data[0];
            ...
            this.notification.create(
              ...
              'PWD Intake Record has successfully deleted.'
            );
          } else {
            this.notification.create(
              "error",
              'Unsuccessfully Deleted',
              'PWD Intake Record unsuccessfully deleted.'
            );
          }
        })
    });
  ...
  onExpandChange(id: number, checked: boolean, index: any): void {
    if (checked) {
      ...
      this.pwdIntakeService.getPwdIntakeEdit(id).subscribe(async data => {
  ...
  applyFilter(){
    ...
    if (this.localStorageDeletedStatus == 0) {
      if (this.localStorageStatus == 0) {
        this.isLoading = true
        this.pwdIntakeService.getAllPwdIntake().subscribe(data => {
          this.isLoading = false
          this.listOfData = data[0]
          this.fcList = this.listOfData
        })
      ...
      this.isLoading = true
      this.pwdIntakeService.getPwdIntakeDeletedOnly().subscribe(data => {
        this.isLoading = false
        this.listOfData = data[0]
        this.fcList = this.listOfData
      })
  ...
  clearSearch(){
    ...
    if (this.localStatusClearDeleted == 0) {
      if (this.localStatusClear == 0) {
        this.pwdIntakeService.getAllPwdIntake().subscribe((value:any) => {
          this.isLoading = false
        ...
        this.pwdIntakeService.getPwdIntake().subscribe((value:any) => {
          this.isLoading = false
          this.listOfData = value[0]
          this.fcList = this.listOfData
          this.suggestions = value[2]
  ...
  generate(value: any) {
    ...
    this.isLoading = true;
    this.pwdIntakeService.getPwdIntakeGenerated(value.filter_type, this.localStatusGenerate, this.localStatusGenerateDeleted, value.thisMonth, value.thisYear, value.monthly, value.monthlyYear, value.yearQuarterly, value.quarter, value.yearly, value.from, value.to).subscribe(data => {
